...
fun DetectorInfoBar(
        ...
        currentTileSize: Int? = null,
        delegateDescription: String? = null,
) {
        Card(
                ...
                Column(modifier = Modifier.padding(12.dp)) {
                        val fpsText =
                                currentDetection?.let {
                                        if (it.fps >= 0f) String.format("%.1f", it.fps) else "-"
                                }
                                        ?: "-"

                        val infText =
                                currentDetection?.let {
                                        if (it.inferenceMs >= 0) "${it.inferenceMs} ms" else "-"
                                }
                                        ?: "-"

                        val objectsText = currentDetection?.afterNmsDetections ?: 0
                        val tileText = currentTileSize?.let { "${it} × ${it}" } ?: "-"
                        val delegateText = delegateDescription ?: "-"
