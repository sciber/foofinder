...
object CameraResolutionUtils {
            ...
            cameraProviderFuture.addListener(
                {
                    try {
                        ...
                        val cameraInfo = cameraProvider.availableCameraInfos.find { info ->
                            cameraSelector.filter(listOf(info)).isNotEmpty()
                        }

                        val resolutions = cameraInfo?.let { info ->
    ...
    fun sortResolutionsByWidth(resolutions: List<Size>): List<Size> {
        return resolutions.sortedWith(compareByDescending<Size> { it.width }.thenByDescending { it.height })
    }
}
